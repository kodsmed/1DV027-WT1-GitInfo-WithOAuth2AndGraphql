...
export const authenticateSessionAndSetLinks = (req: express.Request, res: express.Response, next: express.NextFunction, activeSessions: ActiveSessions) => {
  ...
  if ((!req.session?.UUID || !activeSessions.has(req.session.UUID)) && (!req.originalUrl.includes('/auth/'))) {
    ...
    controller.displayHomePage(req, res, next)
  } else {
