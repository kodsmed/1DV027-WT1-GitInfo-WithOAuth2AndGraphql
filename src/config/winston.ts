 ...
 * @module winston
 * @author Jimmy Karlsson <jk224jv@student.lnu.se> // Translated to TypeScript
 * @contributor Mats Loock <mats.loock@lnu.se> // Wrote the original code in JavaScript
 */
...
// Extend the Transports type with decolorize boolean property.
interface ExtendedFileTransportOptions extends Transport.TransportStreamOptions {
  filename: string;
  decolorize?: boolean;
}
...
if (process.env.LOGGER_COMBINED_LOG_FILE) {
  const extendedFileTransport = new transports.File({
    filename: process.env.LOGGER_COMBINED_LOG_FILE as string,
    decolorize: true, // Now recognized
    format: combine(
      baseFormat,
      decolorize(),
      json()
