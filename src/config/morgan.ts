 ...
 * @contributor Mats Loock <mats.loock@lun.se> // Wrote the original code in JavaScript
 */
...
morgan.token('statusColor', (req: Request, res: Response, args) => {
  // Get the status code if response written. If there is no response written, then get the status code from the headers.
  // Set the status to 0 by default = no color, if there is no response written and no headers.
  const status = res.headersSent || Boolean(res.header) ? res.statusCode : 0
